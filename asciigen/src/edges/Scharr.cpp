        ...
        for (int cx = 0; cx < cols; cx++) {
            ...
            for (int py = 0; py < sub; py++) {
                for (int px = 0; px < sub; px++) {
                    ...
                    const float gx = 3.f * (lumaAt(plane, x + 1, y + 1) - lumaAt(plane, x - 1, y + 1))
                                   + 10.f * (lumaAt(plane, x + 1, y) - lumaAt(plane, x - 1, y))
                                   + 3.f * (lumaAt(plane, x + 1, y - 1) - lumaAt(plane, x - 1, y - 1));

                    const float gy = 3.f * (lumaAt(plane, x + 1, y + 1) - lumaAt(plane, x + 1, y - 1))
                                   + 10.f * (lumaAt(plane, x, y + 1) - lumaAt(plane, x, y - 1))
                                   + 3.f * (lumaAt(plane, x - 1, y + 1) - lumaAt(plane, x - 1, y - 1));
                    ...
                    const float mag = std::sqrt(gx * gx + gy * gy) / kMaxResponse;
                    ...
                    float angle = std::atan2(gy * scaleY, gx * scaleX) + kPi * 0.5f;
            ...
            out.magnitude[i] = std::min(peak, 1.f);
